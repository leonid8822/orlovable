...
9. **Мониторинг FAL.ai и ошибок генерации** (ОБЯЗАТЕЛЬНО проверять при проблемах):
   ```bash
   # Проверить статус FAL.ai (ключ, баланс, доступность)
   curl "https://olai.onrender.com/api/health/fal-status"

   # Полная проверка здоровья системы
   curl "https://olai.onrender.com/api/health/system"

   # Проверить последние ошибки генерации
   curl "https://olai.onrender.com/api/logs?source=generation&level=error&limit=10"
   ```

   **Типовые ошибки и решения:**
   - `403 Forbidden` → FAL_KEY невалидный или истек. Обновить в Render Environment
   - `429 Rate Limit` → Слишком много запросов. Подождать 1 минуту
   - `insufficient_balance` → Закончились кредиты на FAL.ai. Пополнить баланс
   - `timeout` → Генерация зависла. Повторить запрос
   - `safety_check_failed` → Контент заблокирован фильтром безопасности

   **Все ошибки генерации автоматически логируются в:**
   - `app_logs` таблица (source: "generation", level: "error")
   - `applications.last_error` (для конкретной заявки)
