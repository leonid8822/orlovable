...
export function SettingsTab({ onSettingsChange }: SettingsTabProps) {
  ...
  const updateFormFactor = (
    key: string,
    field: keyof FormFactorSettings,
    value: string
  ) => {
    setSettings((prev) => ({
      ...prev,
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(settings.available_models || defaultModels).map(
                ([key, model]) => (
                  <div
                    key={key}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant="secondary">
                        {model.cost_per_image_cents}¢/img
                      </Badge>
                    </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setSettings((prev) => ({ ...prev, num_images: parseInt(v) }))
                }
              >
        ...
        <CardContent>
          <Textarea
            ...
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                flat_pendant_prompt: e.target.value,
              }))
            }
            rows={12}
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Переменные: {"{form_label}"}, {"{user_wishes}"}, {"{form_addition}"},
            {"{form_shape}"}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            Используется для тем: main, kids, totems (круглые, овальные,
            контурные кулоны)
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <Textarea
            ...
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                volumetric_pendant_prompt: e.target.value,
              }))
            }
            rows={12}
            className="font-mono text-sm"
          ...
          />
          <p className="text-xs text-muted-foreground mt-2">
            Переменные: {"{object_description}"}, {"{user_wishes}"},
            {"{size_dimensions}"}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            Используется для темы: custom (произвольные 3D объекты - игрушки,
            статуэтки)
          </p>
        </CardContent>
      </Card>
            ...
            <CardContent>
              <Textarea
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    main_prompt_no_image: e.target.value,
                  }))
                }
                rows={6}
                className="font-mono text-xs"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    value={value.label}
                    onChange={(e) =>
                      updateFormFactor(key, "label", e.target.value)
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateFormFactor(key, "description", e.target.value)
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="rectangle-vertical">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      updateFormFactor(key, "addition", e.target.value)
                    }
                    rows={3}
                    className="text-sm"
                  />
                ...
                <div className="space-y-2">
                  <label className="text-sm text-muted-foreground">
                    Описание формы (shape)
                  </label>
                  <Textarea
                    value={value.shape || ""}
                    onChange={(e) =>
                      updateFormFactor(key, "shape", e.target.value)
                    }
                    rows={3}
                    className="text-sm"
                  />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    value={value.label}
                    onChange={(e) =>
                      updateMaterial(key, "label", e.target.value)
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      updateMaterial(key, "enabled", v === "true")
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {Object.entries(settings.sizes || {}).map(
            ([materialKey, materialSizes]) => (
              <div key={materialKey} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {Object.entries(materialSizes || {}).map(
                    ([sizeKey, sizeValue]) => (
                      <div
                        key={sizeKey}
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            max={100}
                            value={
                              sizeValue.depositPercent ??
                              (materialKey === "gold" ? 30 : 50)
                            }
                            onChange={(e) =>
                              updateSize(
                                materialKey,
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-4 border rounded-lg space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Точка крепления кулона (0,0 = верхний левый угол, 1,1 = нижний
                правый)
              </p>
            </div>
